...
class ReadableLightStatusTests(unittest.TestCase):
    ...
    def test_a_readable_room_that_did_not_answer_is_unreachable_not_unreadable(self):
        # Opposite fixes: "the strip can't tell me anything back" is a permanent
        # fact about Govee hardware, "I couldn't reach it" means go check the
        # wall switch. A Tapo bulb that normally answers and did not is the
        # second, and saying the first about it is a false claim.
        service = self._readable(None)

        reply = _dispatch_lights({"action": "light_status"}, service, LightShadow())

        self.assertIn("couldn't reach", reply)
        self.assertNotIn("can't tell me anything back", reply)

    def test_a_write_only_room_keeps_the_permanent_hedge(self):
        # The Govee strip never answers by design. Reporting it as unreachable
        # every time would send him looking for a fault that does not exist.
        service = _svc()  # bare Mock: can_read_state is not True, so no read

        reply = _dispatch_lights({"action": "light_status"}, service, LightShadow())

        self.assertIn("can't tell me anything back", reply)
        self.assertNotIn("couldn't reach", reply)
