...
class TapoTransport:
    ...
    async def _with_device(self, host: str, action):
        ...
        The `update()` is not optional even for a write: python-kasa populates the
        module list from the device's own component negotiation, so
        `device.modules` is empty until it has run.
        """
        ...
        try:
            await device.update()
            return await action(device, device.modules.get(Module.Light))
        finally:
